...
public class admin_UserList : Page, IRequiresSessionState
{
	...
	{
		this.gridview1.DataSource = this.db.RunProcGetDataView("select B.name as setMealName,B.point as dan,  A.* from Member as A,setMeal as B where A.setMealId=B.id and status=0 and A.agencyName='" + this.Session["user"].ToString() + "' order by A.id");
	...
	protected void gridview1_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		List<MemberInfo> list = MemberInfo.ReadList(string.Concat(new object[]
		{
			...
			(int)this.gridview1.DataKeys[e.RowIndex].Value,
			" and agencyName='",
			this.Session["user"],
			"'"
		}));
