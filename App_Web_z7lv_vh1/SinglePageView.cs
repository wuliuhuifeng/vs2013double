...
public class SinglePageView : Page, IRequiresSessionState
{
	...
	public void BindData()
	{
		...
		int recordConnt = 0;
		this.repeater1.DataSource = this.db.RunProcGetDataView(string.Concat(new object[]
		{
			"select * from article where sortId=",
		...
		MySqlDataReader mySqlDataReader = this.db.RunProcGetReader("select B.moduleTableName as moduleName, A.* from infosort as A,moduleList as B where A.moduleId=B.id and pid<>0 and A.moduleId=13 order by A.sortOrder ");
