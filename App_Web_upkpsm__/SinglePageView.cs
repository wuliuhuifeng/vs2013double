...
public class SinglePageView : Page, IRequiresSessionState
{
	...
	public void BindData()
	{
		...
		{
			sQL = "select B.sortName as sortName,B.moduleId as moduleId, A.* from article as A,infoSort as B where A.sortId=B.id and A.sortId=" + int.Parse(base.Request["sortId"]);
		}
		if (base.Request["id"] != null)
		{
			sQL = "select B.sortName as sortName,B.moduleId as moduleId, A.* from article as A,infoSort as B where A.sortId=B.id and A.Id=" + int.Parse(base.Request["Id"]);
		}
		...
		if (mySqlDataReader.Read())
		{
			...
			this.articleTitle = mySqlDataReader["title"].ToString();
			this.content = mySqlDataReader["content"].ToString();
		}
		mySqlDataReader.Close();
		base.Title = this.articleTitle;
