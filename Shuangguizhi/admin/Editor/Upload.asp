...
<input type=file name=uploadfile id=uploadfile size=1 style="width:100%" onchange="originalfile.value=this.value;<% If sType="IMAGE" Then Response.Write("parent.reloadImg();") %>">
...
'----2006-12-31
Function DateFolder(ByVal val)
	Dim dt,y,m,d
	dt = Now()
	y  = Year(dt)
	m  = Right("00"& Month(dt),2)
	d  = Right("00"& Day(dt),2)
	Select Case val
		Case "1" DateFolder = y &"/"
		Case "2" DateFolder = y &"/"& m &"/"
		Case "3" DateFolder = y &"/"& m &"/"& d &"/"
	End Select 
End Function 
Sub CheckFolderExists(ByVal root,ByVal Up_folder) '检查文件夹是否存在，不存在则创建
	Dim TempFolder,arrFolder,fso,i
	TempFolder= Server.Mappath(root)
	Set fso = Server.CreateObject("Scripting.FileSystemObject")
	If Not Fso.Folderexists(TempFolder & Up_folder) Then
		arrFolder = Split(Up_folder,"/")
		For i=0 To UBound(arrFolder)-1
			TempFolder = TempFolder &"/"& arrFolder(i)
...
Function getDateFolder()
	Dim strDateFolder
	strDateFolder = DateFolder(nAutoDir)
	CheckFolderExists sUploadDir,strDateFolder
	getDateFolder = strDateFolder
End Function 
'----
' 无组上传类
Sub DoUpload_Class()
	...
	sOriginalFileName = Replace(sOriginalFileName,"'","\'") '----修正源文件名包含“'” 2007-1-1
	sSaveFileName = GetRndFileName(sFileExt)
	sSaveFileName = getDateFolder & sSaveFileName '----自动目录 2006-12-31
...
Sub InitUpload()
	...
	oRs.Close
	' 任何情况下都不允许上传asp脚本文件
	sAllowExt = sAllowExtFilter(sAllowExt) '----2007-1-1
End Sub

'*************************************
'过滤文件名字
'*************************************
Function sAllowExtFilter(ByVal sVal)
	If IsEmpty(sVal) Then Exit Function
	sAllowExtFilter = Ucase(sVal)
	sAllowExtFilter = Replace(sAllowExtFilter,"ASP","")
	sAllowExtFilter = Replace(sAllowExtFilter,"ASA","")
	sAllowExtFilter = Replace(sAllowExtFilter,"ASPX","")
	sAllowExtFilter = Replace(sAllowExtFilter,"CER","")
	sAllowExtFilter = Replace(sAllowExtFilter,"CDX","")
	sAllowExtFilter = Replace(sAllowExtFilter,"HTR","")
End Function
...
Function ReplaceRemoteUrl(sHTML, sExt)
	...
	For i = 1 To n
		...
		SaveFileName = getDateFolder & SaveFileName '----自动目录 2006-12-31
		If SaveRemoteFile(SaveFileName, a_RemoteUrl(i)) = True Then
			...
			If sValue="" Then 
				sValue = SaveFileName
			Else 
				sValue = sValue &"|"& SaveFileName
			End If 

		End If
	Next

	Call updateRemoteUpfileList(sValue)

	ReplaceRemoteUrl = s_Content
End Function
Sub updateRemoteUpfileList(ByVal sValue)
	Response.Write("<script language='javascript'>"&vbCrLf) '2007-1-2
	Response.Write("	var sValue='"& sValue &"';"&vbCrLf)
	Response.Write("	var o=parent.parent.document.getElementById('upfile');"&vbCrLf)
	Response.Write("	if (o){"&vbCrLf)
	Response.Write("		if (o.value.length!=0){o.value+='|';}"&vbCrLf)
	Response.Write("		o.value+=sValue.toLowerCase();"&vbCrLf)
	Response.Write("	}"&vbCrLf)
	Response.Write("	var oo=parent.parent.document.getElementById('upfileImg')"&vbCrLf)
	Response.Write("	if(oo) {"&vbCrLf)
	Response.Write("		var strExt = '|.jpg|.jpeg|.gif|.bmp|.png|';"&vbCrLf)
	Response.Write("		var aUpfiles=sValue.split('|');"&vbCrLf)
	Response.Write("		for (i=0;i<aUpfiles.length;i++){"&vbCrLf)
	Response.Write("			sValue = aUpfiles[i];"&vbCrLf)
	Response.Write("			var strFileExt = '|'+sValue.substring(sValue.lastIndexOf('.'),sValue.length).toLowerCase()+'|';"&vbCrLf)
	Response.Write("			if (strExt.indexOf(strFileExt)!=-1){"&vbCrLf)
	Response.Write("				var opt=parent.parent.document.createElement('OPTION');"&vbCrLf)
	Response.Write("				oo.options.add(opt);"&vbCrLf)
	Response.Write("				opt.innerHTML=sValue;"&vbCrLf)
	Response.Write("				opt.value=sValue;"&vbCrLf)
	Response.Write("			}"&vbCrLf)
	Response.Write("		}"&vbCrLf)
	Response.Write("	}"&vbCrLf)
