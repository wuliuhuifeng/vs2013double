...
// 上传文件名列表
function updateUpfileList(sValue){
	if (d_checkfromurl.checked){return;}//来源网络则退出
	sValue=sValue.split('upload/')[1].toLowerCase();
	///* 上传的文件名保存为 val1|val2 形式，并赋值给upfile
	var o=dialogArguments.eWebEditor.parent.parent.document.getElementById("upfile")
	if(o) {
		if (o.value.length!=0){o.value+='|';}
		o.value+=sValue;
	}
	///* 上传的文件名填充下拉列表给upfileImg，仅图片
	var oo=dialogArguments.eWebEditor.parent.parent.document.getElementById("upfileImg")
	if(oo) {
		var strExt = "|.jpg|.jpeg|.gif|.bmp|.png|";
		var strFileExt = '|'+sValue.substring(sValue.lastIndexOf("."),sValue.length).toLowerCase()+'|';
		if (strExt.indexOf(strFileExt)!=-1){
			var opt=dialogArguments.eWebEditor.parent.parent.document.createElement("OPTION");
			oo.options.add(opt);
...
// 上传文件名列表
function updateRemoteUpfileList(sValue){
	//upload.asp line:389
}
